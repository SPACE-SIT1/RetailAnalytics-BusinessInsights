...
-- Section 1: Normalize raw transaction fields and flag rows with critical issues
...
-- Section 2: Build the primary cleaned transaction table for downstream analysis
...
-- Section 3: Store rejected rows separately for audit and data-quality review
...
-- Section 4: Surface unmatched foreign keys against the original master tables
...
-- Section 5: Impute missing transaction values using invoice consistency, SKU parsing, and product-level defaults
...
-- Section 6: Create final employee records, including fallback placeholders for unknown store-level assignments
...
-- Section 7: Create final customer records and append an explicit UNKNOWN_CUSTOMER member
...
-- Section 8: Create final product records and append an explicit UNKNOWN_PRODUCT member
...
-- Section 9: Create final store records and append an explicit UNKNOWN_STORE member
...
-- Section 10: Assemble the final transaction fact table used for Python EDA and Tableau analysis
...
-- Section 11: Re-check foreign-key integrity after final fallback records are added
DROP VIEW IF EXISTS v_invalid_foreign_keys;
...
-- Section 12: Remove intermediate staging objects once final tables are materialized
DROP TABLE IF EXISTS transactions_clean;
DROP TABLE IF EXISTS transactions_rejected;
DROP TABLE IF EXISTS transactions_imputed;
DROP VIEW IF EXISTS v_transactions_clean;

-- Section 13: Publish a compact cleaning summary for quick validation and QA checks
